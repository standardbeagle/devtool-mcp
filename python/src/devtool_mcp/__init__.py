...
offering project detection, process management, reverse proxy with
frontend diagnostics, and more.
...
def get_cache_dir() -> Path:
    ...
    cache_dir = base / "devtool-mcp" / VERSION
...
def get_binary_path() -> Path:
    """Get the path to the devtool-mcp binary, downloading if necessary."""
    ...
    print(f"Downloading devtool-mcp v{VERSION}...", file=sys.stderr)
    ...
    try:
        with httpx.Client(follow_redirects=True, timeout=60.0) as client:
            ...
            print(f"Successfully installed devtool-mcp to {binary_path}", file=sys.stderr)

    except httpx.HTTPError as e:
        print(f"Failed to download devtool-mcp: {e}", file=sys.stderr)
        ...
        print("  git clone https://github.com/standardbeagle/devtool-mcp.git", file=sys.stderr)
        print("  cd devtool-mcp", file=sys.stderr)
...
def main() -> None:
    """Entry point for the devtool-mcp command."""
    binary_path = get_binary_path()
    try:
